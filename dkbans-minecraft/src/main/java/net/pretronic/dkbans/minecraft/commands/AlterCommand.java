...
public class AlterCommand extends BasicCommand {
    ...
    public void execute(CommandSender sender, String[] arguments) {
        if(arguments.length < 1){
            sender.sendMessage(Messages.COMMAND_ALERT_HELP);
            return;
        }
        String text = CommandUtil.readStringFromArguments(arguments,0);
        DKBans.getInstance().getBroadcastManager().sendBroadcast(text);
    }
}
